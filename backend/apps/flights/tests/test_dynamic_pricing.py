...
from apps.flights.models import (
    ...
    CabinClass, RefundType, RouteFareClass, Fare,
    DynamicPricingConfig, HolidayEvent, DynamicPriceLog
)
from apps.flights.services_pricing import DynamicPricingStrategy, reevaluate_route_fares_dynamically
...
class DynamicPricingEngineTest(TestCase):
    def setUp(self):
        ...
        self.config = DynamicPricingConfig.objects.create(
            ...
            holiday_multiplier=Decimal("1.30"),
            demand_surge_per_booking=Decimal("0.05"),
            max_surge_cap=Decimal("1.80"),
    ...
    def test_dynamic_pricing_breakdown_weekend(self):
        ...
        self.assertEqual(breakdown["final_calculated_price"], "12000.00")
    ...
    def test_max_surge_cap(self):
        strategy = DynamicPricingStrategy()
        today = date.today()
        # 50 mock bookings * 0.05 = 2.5 surge, capped at max_surge_cap (1.80)
        breakdown = strategy.calculate_price_breakdown(
            route_fare=self.route_fare_econ,
            flight_date=today,
        ...
        self.assertEqual(breakdown["combined_multiplier"], "1.80")
        self.assertEqual(breakdown["final_calculated_price"], "18000.00")
    ...
    def test_simulator_api_endpoint(self):
        response = self.client.post(
            "/api/v2/dynamic-pricing-config/simulate/",
            {
