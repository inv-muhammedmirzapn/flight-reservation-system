...
print("Starting Hamburg flights seeding...")

# 1. Ensure Country Germany exists
germany, _ = Country.objects.get_or_create(
    iso_code="DE",
    defaults={"name": "Germany"}
)

# 2. Ensure Hamburg Airport (HAM) exists
...
# 3. Ensure Delhi Airport (DEL) exists
...
for d_offset in range(num_days):
    ...
    for idx, (fno, (fr, ac, d_hrs, d_mins, base_fare)) in enumerate(created_routes.items()):
        # Schedule departure times (e.g. 02:30, 06:15, 14:00)
        ...
        # Decide if this flight is FULL (0 seats available)
        # Make ~1 out of 3 flights have 0 seats to allow waitlist testing!
        ...
        Fare.objects.update_or_create(
            ...
            defaults={
                ...
                "price": Decimal(str(base_fare * 2.8)),
                "currency": "INR",
        ...
        Fare.objects.update_or_create(
            ...
            defaults={
                ...
                "price": Decimal(str(base_fare * 5.2)),
                "currency": "INR",
...
print(f"Successfully created {count_created} flights from DEL to HAM starting from Aug 5th, 2026 ({zero_seat_count} flights fully booked with 0 seats).")
